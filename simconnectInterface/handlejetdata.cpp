...
void SimconnectInterface::handleJetSingleEngdata(SIMCONNECT_RECV_SIMOBJECT_DATA *pObjData)
{
    ...
    //n1
    if(fabs(d_lastJetData.engine1N1 - newData->engine1N1) >= 0.05)
    {
    ...
    //n2
    if(fabs(d_lastJetData.engine1N2 - newData->engine1N2) >= 0.05)
    {
    ...
    //itt
    if(fabs(d_lastJetData.engine1Itt - newData->engine1Itt) >= 0.5)
    {
    ...
    {
        if(fabs(d_lastJetData.commonData.fuelQtyTotal - newData->commonData.fuelQtyTotal) >= 0.02)
        {
    ...
    {
        if(fabs(d_lastJetData.commonData.fuelQtyLeft - newData->commonData.fuelQtyLeft) >= 0.02)
        {
        ...
        if(fabs(d_lastJetData.commonData.fuelQtyRight - newData->commonData.fuelQtyRight) >= 0.02)
        {
    ...
    {
        if(fabs(d_lastJetData.commonData.engine1FuelFlowVolume - newData->commonData.engine1FuelFlowVolume) >= 0.1)
        {
            d_lastJetData.commonData.engine1FuelFlowVolume = newData->commonData.engine1FuelFlowVolume;
    ...
    {
        if(fabs(d_lastJetData.commonData.engine1FuelFlowWeight - newData->commonData.engine1FuelFlowWeight) >= 0.00003)
        {
            d_lastJetData.commonData.engine1FuelFlowWeight = newData->commonData.engine1FuelFlowWeight;
    ...
    //oil temp
    if(fabs(d_lastJetData.commonData.engine1OilTemp - newData->commonData.engine1OilTemp) >= 0.1)
    {
    ...
    //oil press
    if(fabs(d_lastJetData.commonData.engine1OilPress - newData->commonData.engine1OilPress) >= 0.8)
    {
...
void SimconnectInterface::handleJetTwinEngdata(SIMCONNECT_RECV_SIMOBJECT_DATA *pObjData)
{
    JetData *newData = reinterpret_cast<JetData *>(&pObjData->dwData);
    QByteArray dataToSend;
    DataIdentifiers id;
    ...
    if(fabs(d_lastJetData.engine2N1 - newData->engine2N1) >= 0.05)
    {
    ...
    if(fabs(d_lastJetData.engine2N2 - newData->engine2N2) >= 0.05)
    {
    ...
    if(fabs(d_lastJetData.engine2Itt - newData->engine2Itt) >= 0.5)
    {
    ...
    if (d_currentAirplaneSettings.fuelFlowByVolume)
    {
        ...
        if(fabs(d_lastJetData.commonData.engine2FuelFlowVolume - newData->commonData.engine2FuelFlowVolume) >= 0.1)
        {
            d_lastJetData.commonData.engine2FuelFlowVolume = newData->commonData.engine2FuelFlowVolume;
    ...
    else
    {
        ...
        if(fabs(d_lastJetData.commonData.engine2FuelFlowWeight - newData->commonData.engine2FuelFlowWeight) >= 0.00003)
        {
            d_lastJetData.commonData.engine2FuelFlowWeight = newData->commonData.engine2FuelFlowWeight;
    ...
    if(fabs(d_lastJetData.commonData.engine2OilTemp - newData->commonData.engine2OilTemp) >= 0.1)
    {
    ...
    if(fabs(d_lastJetData.commonData.engine2OilPress - newData->commonData.engine2OilPress) >= 0.8)
    {
...
void SimconnectInterface::handleJetQuadEngdata(SIMCONNECT_RECV_SIMOBJECT_DATA *pObjData)
{
    JetData *newData = reinterpret_cast<JetData *>(&pObjData->dwData);
    QByteArray dataToSend;
    DataIdentifiers id;
    ...
    if(fabs(d_lastJetData.engine3N1 - newData->engine3N1) >= 0.05)
    {
    ...
    if(fabs(d_lastJetData.engine4N1 - newData->engine4N1) >= 0.05)
    {
    ...
    if(fabs(d_lastJetData.engine3N2 - newData->engine3N2) >= 0.05)
    {
    ...
    if(fabs(d_lastJetData.engine4N2 - newData->engine4N2) >= 0.05)
    {
    ...
    if(fabs(d_lastJetData.engine3Itt - newData->engine3Itt) >= 0.5)
    {
    ...
    if(fabs(d_lastJetData.engine4Itt - newData->engine4Itt) >= 0.5)
    {
    ...
    if (d_currentAirplaneSettings.fuelFlowByVolume)
    {
        if(fabs(d_lastJetData.commonData.engine1FuelFlowVolume + d_lastJetData.commonData.engine2FuelFlowVolume - newData->commonData.engine1FuelFlowVolume - newData->commonData.engine2FuelFlowVolume) >= 0.2)
        {
            d_lastJetData.commonData.engine1FuelFlowVolume = newData->commonData.engine1FuelFlowVolume;
            d_lastJetData.commonData.engine2FuelFlowVolume = newData->commonData.engine2FuelFlowVolume;
            newData->commonData.engine1FuelFlowVolume = (newData->commonData.engine1FuelFlowVolume + newData->commonData.engine2FuelFlowVolume) / 2.0;
            id = DataIdentifiers::ENGINE1_FUEL_FLOW;
            dataToSend.append(reinterpret_cast<char *>(&id), sizeof(id));
            dataToSend.append(reinterpret_cast<char *>(&newData->commonData.engine1FuelFlowVolume), sizeof(newData->commonData.engine1FuelFlowVolume));
        }
        if(fabs(d_lastJetData.commonData.engine3FuelFlowVolume + d_lastJetData.commonData.engine4FuelFlowVolume - newData->commonData.engine3FuelFlowVolume - newData->commonData.engine4FuelFlowVolume) >= 0.2)
        {
            d_lastJetData.commonData.engine3FuelFlowVolume = newData->commonData.engine3FuelFlowVolume;
            d_lastJetData.commonData.engine4FuelFlowVolume = newData->commonData.engine4FuelFlowVolume;
            newData->commonData.engine3FuelFlowVolume = (newData->commonData.engine3FuelFlowVolume + newData->commonData.engine4FuelFlowVolume) / 2.0;
            id = DataIdentifiers::ENGINE2_FUEL_FLOW;
            dataToSend.append(reinterpret_cast<char *>(&id), sizeof(id));
            dataToSend.append(reinterpret_cast<char *>(&newData->commonData.engine3FuelFlowVolume), sizeof(newData->commonData.engine3FuelFlowVolume));
        }
    }
    else
    {
        if(fabs(d_lastJetData.commonData.engine1FuelFlowWeight + d_lastJetData.commonData.engine2FuelFlowWeight - newData->commonData.engine1FuelFlowWeight - newData->commonData.engine2FuelFlowWeight) >= 0.00006)
        {
            d_lastJetData.commonData.engine1FuelFlowWeight = newData->commonData.engine1FuelFlowWeight;
            d_lastJetData.commonData.engine2FuelFlowWeight = newData->commonData.engine2FuelFlowWeight;
            newData->commonData.engine1FuelFlowWeight = (newData->commonData.engine1FuelFlowWeight + newData->commonData.engine2FuelFlowWeight) * 1800.0;
            id = DataIdentifiers::ENGINE1_FUEL_FLOW;
            dataToSend.append(reinterpret_cast<char *>(&id), sizeof(id));
            dataToSend.append(reinterpret_cast<char *>(&newData->commonData.engine1FuelFlowWeight), sizeof(newData->commonData.engine1FuelFlowWeight));
        }
        if(fabs(d_lastJetData.commonData.engine3FuelFlowWeight + d_lastJetData.commonData.engine4FuelFlowWeight - newData->commonData.engine3FuelFlowWeight - newData->commonData.engine4FuelFlowWeight) >= 0.00006)
        {
            d_lastJetData.commonData.engine3FuelFlowWeight = newData->commonData.engine3FuelFlowWeight;
            d_lastJetData.commonData.engine4FuelFlowWeight = newData->commonData.engine4FuelFlowWeight;
            newData->commonData.engine3FuelFlowWeight = (newData->commonData.engine3FuelFlowWeight + newData->commonData.engine4FuelFlowWeight) * 1800.0;
            id = DataIdentifiers::ENGINE2_FUEL_FLOW;
            dataToSend.append(reinterpret_cast<char *>(&id), sizeof(id));
            dataToSend.append(reinterpret_cast<char *>(&newData->commonData.engine3FuelFlowWeight), sizeof(newData->commonData.engine3FuelFlowWeight));
        }
    }

    //oil temp
    if(fabs(d_lastJetData.commonData.engine1OilTemp + d_lastJetData.commonData.engine2OilTemp - newData->commonData.engine1OilTemp - newData->commonData.engine2OilTemp) >= 0.2)
    {
        d_lastJetData.commonData.engine1OilTemp = newData->commonData.engine1OilTemp;
        d_lastJetData.commonData.engine2OilTemp = newData->commonData.engine2OilTemp;
    ...
    if(fabs(d_lastJetData.commonData.engine3OilTemp + d_lastJetData.commonData.engine4OilTemp - newData->commonData.engine3OilTemp - newData->commonData.engine4OilTemp) >= 0.2)
    {
    ...
    //oil press
    if(fabs(d_lastJetData.commonData.engine1OilPress + d_lastJetData.commonData.engine2OilPress - newData->commonData.engine1OilPress - newData->commonData.engine2OilPress) >= 1.6)
    {
        d_lastJetData.commonData.engine1OilPress = newData->commonData.engine1OilPress;
        d_lastJetData.commonData.engine2OilPress = newData->commonData.engine2OilPress;
    ...
    if(fabs(d_lastJetData.commonData.engine3OilPress + d_lastJetData.commonData.engine4OilPress - newData->commonData.engine3OilPress - newData->commonData.engine4OilPress) >= 1.6)
    {
