...
void SimconnectThread::handleAirspeedData(const PfdAirspeedStruct &newData)
{
    ...
    dataToSend.append(reinterpret_cast<const char *>(&newData.airspeed),
    ...
    if (fabs(d_lastAirspeedData.max_speed - newData.max_speed) >= 0.09)
    {
        ...
        dataToSend.append(reinterpret_cast<const char *>(&newData.max_speed),
                          sizeof(newData.max_speed));
    }
    ...
    if (d_lastAirspeedData.true_airspeed != newData.true_airspeed)
    {
        ...
        dataToSend.append(reinterpret_cast<const char *>(&id), sizeof(id));
        dataToSend.append(reinterpret_cast<const char *>(&newData.true_airspeed),
                          sizeof(newData.true_airspeed));
    }
    ...
    if (d_lastAirspeedData.ref_speed != newData.ref_speed)
    {
        ...
        dataToSend.append(reinterpret_cast<const char *>(&id), sizeof(id));
        dataToSend.append(reinterpret_cast<const char *>(&newData.ref_speed),
                          sizeof(newData.ref_speed));
    }
