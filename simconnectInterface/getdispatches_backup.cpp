...
void SimconnectInterface::getDispatches()
{
            ...
            case SIMCONNECT_RECV_ID_EVENT:
            {
                ...
                if (evt->uEventID == EVENT_4SEC_ID)
                {
                    if (updateAircraft)
                    {
                        ...
                        setupAircraftData(d_newAircraft);
                        updateAircraft = false;
                        SimConnect_RequestDataOnSimObject(d_simConnectHandle, ENGINE_DATA_REQUEST_ID, ENGINE_DATA_DEFINITION_ID, SIMCONNECT_OBJECT_ID_USER, SIMCONNECT_PERIOD_VISUAL_FRAME, SIMCONNECT_CLIENT_DATA_REQUEST_FLAG_CHANGED);
                    }

                    uint8_t atomicValue = thread_communicator.load();

                    if (atomicValue != lastAtomicValue)
                    {
                        lastAtomicValue = atomicValue;
                        if((atomicValue & 1) == 1)
                        {
                            quit = true;
                            break;
                        ...
                        d_newAircraft = static_cast<ImplementedAircraft>(atomicValue >> 1);
                        updateAircraft = true;
                    }
                ...
                else if (evt->uEventID == SIM_START_EVENT_ID)
                {
                    ...
                    emit sendData(QByteArray(reinterpret_cast<char *>(&id), 1));
                }
