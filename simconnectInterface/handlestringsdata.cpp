...
void SimconnectInterface::handleStringsData(SIMCONNECT_RECV_SIMOBJECT_DATA *pObjData)
{
    StringsDataStruct newData(reinterpret_cast<StringsRawDataStruct *>(&pObjData->dwData));

    DataIdentifiers id = DataIdentifiers::CURRENT_LEG_TO;
    ...
    if (d_lastStringsData.gps_wp_prev_id != newData.gps_wp_prev_id)
    {
        ...
        id = DataIdentifiers::CURRENT_LEG_FROM;
        dataToSend.append(reinterpret_cast<char *>(&id), sizeof(id));
...
void SimconnectInterface::sendBlankStringsData()
{
